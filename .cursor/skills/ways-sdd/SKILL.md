...
Run `npx ways sdd start the arguments the human gave with this skill`, then work phase by phase: fill `.ways/sdd/<id>/<phase>.md` with Goal, Evidence, Decision and Gate, and certify with `npx ways sdd advance`; never edit a certified phase file. At a supervised human gate stop and ask the human to run `npx ways approve` in their own terminal; you cannot approve and must not touch `approvals/` or `reviews/`. During review give ways-reviewer the digest from `npx ways review digest`; it must appear in the review JSON, and any later edit invalidates the review.
